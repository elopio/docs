...
The Fork Threshold is the amount of [REP](#rep) that must be [Staked](#dispute-stake) on a [Dispute Crowdsourcer](#dispute-crowdsourcer) in order to cause a [Market](#market) to [Fork](#fork). In Augur, if 2.5% of all existing REP is Staked on the Dispute Crowdsourcer of a Market, that Market will Fork.
