...
MarketCreated             | [Augur](https://github.com/AugurProject/augur-core/blob/master/source/contracts/Augur.sol) | `marketCreator` has created `market` in `universe` for a price of `marketCreationFee`. | universe, market, marketCreator | marketCreationFee, extraInfo
...
OrderCreated              | [Augur](https://github.com/AugurProject/augur-core/blob/master/source/contracts/Augur.sol) | `creator` placed an Order (with ID `orderId`) in `universe` for the Share Token at address `shareToken` in the Trade Group `tradeGroupId`. | universe, shareToken, creator | orderId, tradeGroupId
OrderFilled               | [Augur](https://github.com/AugurProject/augur-core/blob/master/source/contracts/Augur.sol) | Order `orderId` in `universe` for Share Token at address `shareToken` was filled by `filler` in Trade Group `tradeGroupId`. The Order Creator escrowed `numCreatorShares` or `numCreatorTokens` in ETH, and `filler` put up `numFillerShares` or `numFillerTokens` in ETH, and `settlementFees` in ETH were spent. | universe, shareToken | filler, orderId, numCreatorShares, numCreatorTokens, numFillerShares, numFillerTokens, settlementFees, tradeGroupId
