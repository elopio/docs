...
MarketCreated             | [Augur](https://github.com/AugurProject/augur-core/blob/master/source/contracts/Augur.sol) | `marketCreator` has created `market` with `topic` in `universe` for a price of `marketCreationFee`. | universe, topic, marketCreator | market, marketCreationFee, extraInfo
