...
<!-- TODO: Update this once Dispute Round changes are finalized. -->
...
ReportSubmitted           | [Augur](https://github.com/AugurProject/augur-core/blob/master/source/contracts/Augur.sol) | `reporter` submitted a Report for `market` in `universe`. `reporter` staked `amountStaked` ETH on Outcome `payoutNumerators` using the Stake Token at address `stakeToken`. | universe, reporter, market | stakeToken, amountStaked, payoutNumerators
...
<!-- TODO: Update this once Dispute Round changes are finalized. -->
UniverseForked            | [Augur](https://github.com/AugurProject/augur-core/blob/master/source/contracts/Augur.sol) | A Market in `universe` has had its Tentative Outcome disputed during the 20th Dispute Round, which has caused `universe` to Fork. | universe |
